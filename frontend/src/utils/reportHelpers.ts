...
export const isInfantAge = (rawAge: any, numAgeInYears?: number | null): boolean => {
  ...
  const numAge = numAgeInYears !== undefined && numAgeInYears !== null ? numAgeInYears : parseFloat(ageStr);
...
export const matchesSelectedMonthAndYear = (
  ...
  if (!rawDateStr || (typeof rawDateStr !== 'string' && typeof rawDateStr !== 'number')) {
    return false;
  }

  const cleanStr = rawDateStr.toString().trim();
  if (
    !cleanStr ||
    cleanStr === 'N/A' ||
    cleanStr.toLowerCase() === 'null' ||
    cleanStr.toLowerCase() === 'undefined'
  ) {
    return false;
  }
  ...
  const targetMonthIndex = monthNames.indexOf(targetMonth.toUpperCase().trim());
  const targetYearNum = parseInt(targetYear.toString().trim(), 10);
  ...
  if (mmddyyyyParts) {
    ...
    return m === targetMonthIndex && y === targetYearNum;
  }
  ...
  // 3. Native Date Object Fallback
  const parsedDate = new Date(cleanStr);
  if (!isNaN(parsedDate.getTime())) {
    return parsedDate.getMonth() === targetMonthIndex && parsedDate.getFullYear() === targetYearNum;
  }
